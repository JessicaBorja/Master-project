...
def update_mask_gripper(fixed_points, curr_pt, mask, directions,
                        frame_img_tuple, out_img_size, pop_last=True,
                        current_obs=None, cam_properties=None):
    # fixed_obs
    fixed_objs = fixed_points.copy()
    # Update masks with fixed_points
    centers, distances = [], []
    (frame_timestep, img) = frame_img_tuple
    if(len(fixed_points) > 1 and pop_last):
        distances = np.array([fp[1] for fp in fixed_points]) - curr_pt
        distances = np.linalg.norm(distances, axis=-1)
        min_dist_idx = np.argmin(distances)
        fixed_objs.pop(min_dist_idx)

    for point_timestep, p in fixed_points:
        # Only add point if it was fixed before seing img
        if(frame_timestep >= point_timestep):
            new_mask, center_px = get_gripper_mask(img, current_obs[:6], p, cam_properties)
...
def update_mask(fixed_points, mask, directions,
                frame_img_tuple, out_img_size, cam):
    # Update masks with fixed_points
    centers = []
    (frame_timestep, img) = frame_img_tuple
    for point_timestep, p in fixed_points:
...
def label_gripper(cam_properties, fixed_points, img_hist, point, viz,
                  save_dict, out_img_size, radius=25, pop_last=True):
    for idx, (fr_idx, im_id, robot_obs, img) in enumerate(img_hist):
        if(im_id not in save_dict):
            ...
            fp_mask, centers_px, fp_directions = \
                update_mask_gripper(fixed_points, robot_obs[:3],
                                    np.zeros((H, W)),
                                    directions,
                                    (fr_idx, img),
                                    out_img_size,
                                    current_obs=robot_obs[:6],
                                    cam_properties=cam_properties,
                                    pop_last=pop_last)
            ...
            mask = overlay_mask(mask, fp_mask, (255, 255, 255))
