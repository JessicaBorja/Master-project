...
class SACWorker(Worker):
    def __init__(self, hyperparameters, eval_config, learn_config,
                 run_id, nameserver, logger):
    ...
    def compute(self, config, budget, working_directory, *args, **kwargs):
        ...
        if("hidden_dim" in config):
            self.hyperparameters['net_cfg'].update({
        ...
        eval_mean_reward, _ = model.evaluate(model.eval_env,
                                             **self.eval_config)
...
def optimize(trial_name, hyperparameters, eval_config, learn_config,
             run_id, nameserver, max_budget=250000, min_budget=50000,
