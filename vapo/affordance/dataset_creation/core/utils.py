...
def save_dict_data(data_dict, directory, sub_dir, save_viz=False):
    ...
    if save_viz:
        data_dir, viz_dir, viz_frames, viz_aff = create_dirs(
            directory, sub_dir, ["data", "viz_direction", "viz_frames", "viz_affordance"]
        )
    else:
...
def instantiate_env(cfg, real_world, new_cfg=True):
