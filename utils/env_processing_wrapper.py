...
class EnvWrapper(gym.ObservationWrapper):
    def __init__(self, env, use_img, use_depth, use_pos,
                 history_length, skip_frames, img_size, train=False):
        super(EnvWrapper, self).__init__(env)
        self.env = env

        # setup to stack images
        self.skip_frames = skip_frames
        self.history_length = history_length
        self.obs_count = 0
        self.img_size = img_size
        self._total_frames = np.zeros(
            ((skip_frames+1)*(self.history_length-1) + 1,
             self.img_size, self.img_size))
        self._indices = [i for i in range(self._total_frames.shape[0])
    ...
    def get_obs_space(self, use_img, use_depth, use_pos):
        if(use_img):
            ...
            # robot_obs= 3 for pos, 3 for angle and 1 for gripper.
            # idk how to define the obs space for this.
            # obs_space_dict = {
            #     "scene_obs":  gym.spaces.Box(low=0, high=1.5, shape=(3,)),
            # # each door has different ranges
